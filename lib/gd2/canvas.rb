...
module GD2
  class Canvas
    ...
    class Line
      ...
      def draw(image, mode)
        ::GD2::GD2FFI.send(:gdImageLine, image.image_ptr, @point_1.x.to_i, @point_1.y.to_i, @point_2.x.to_i, @point_2.y.to_i, mode.to_i)
    ...
    class Rectangle
      ...
      def draw(image, mode)
        ::GD2::GD2FFI.send(draw_sym, image.image_ptr, @point_1.x.to_i, @point_1.y.to_i, @point_2.x.to_i, @point_2.y.to_i, mode.to_i)
    ...
    class Arc
      def initialize(center, width, height, range)
        ...
        @width = width
        @height = height
        @range = Range.new(360.degrees - range.end, 360.degrees - range.begin, range.exclude_end?)
      end

      def draw(image, mode)
        ::GD2::GD2FFI.gdImageArc(
          image.image_ptr,
          @center.x.to_i,
          @center.y.to_i,
          @width.to_i,
          @height.to_i,
          @range.begin.to_degrees.round.to_i,
          @range.end.to_degrees.round.to_i,
          mode.to_i
        )
    ...
    class Ellipse
      ...
      def draw(image, mode)
        ::GD2::GD2FFI.send(:gdImageArc, image.image_ptr, @center.x.to_i, @center.y.to_i, @width.to_i, @height.to_i, 0, 360, mode.to_i)
    ...
    class FilledEllipse < Ellipse
      def draw(image, mode)
        ::GD2::GD2FFI.send(:gdImageFilledEllipse, image.image_ptr, @center.x.to_i, @center.y.to_i, @width.to_i, @height.to_i, mode.to_i)
    ...
    class TextCircle
      def initialize(font, point, radius, text_radius, fill_portion, top, bottom)
        @font = font
        @point = point
        @radius = radius
      ...
      def draw(image, color)
        @font.draw_circle(image.image_ptr, @point.x, @point.y, @radius, @text_radius, @fill_portion, @top, @bottom, color)
      end
    end

    attr_reader :color, :thickness, :style, :brush, :tile, :dont_blend, :transformation_matrix
    ...
    def style=(ary)
      return unless (@style = ary)

      ::GD2::GD2FFI.send(:gdImageSetStyle, @image.image_ptr,
        ary.map do |c|
          if c
            c == true ? -1 : @image.color2pixel(c)
          else
            TRANSPARENT
          end
        end, ary.length)
    ...
    def brush=(image)
      ::GD2::GD2FFI.send(:gdImageSetBrush, @image.image_ptr, image.image_ptr) if (@brush = image)
    end

    def tile=(image)
      ::GD2::GD2FFI.send(:gdImageSetTile, @image.image_ptr, image.image_ptr) if (@tile = image)
    end
    ...
    def affine_transform(a, b, c, d, tx, ty)
      ...
      begin
        @transformation_matrix = Matrix[[a, b, 0], [c, d, 0], [tx, ty, 1]] * @transformation_matrix
        yield
      ensure
    ...
    def translate(tx, ty)
      affine_transform(1, 0, 0, 1, tx, ty) do |*block_args|
        yield(*block_args)
      end
    end

    def scale(sx, sy = sx)
      affine_transform(sx, 0, 0, sy, 0, 0) do |*block_args|
        yield(*block_args)
      end
    end
    ...
    def rotate(angle)
      ...
      affine_transform(cos, sin, -sin, cos, 0, 0) do |*block_args|
        yield(*block_args)
      end
    end

    def cartesian
      affine_transform(1, 0, 0, -1, 0, @image.height - 1) do |*block_args|
        yield(*block_args)
      end
    end
    ...
    def move(x, y)
      @point.transform!(Matrix[[1, 0, 0], [0, 1, 0], [x, y, 1]] * @transformation_matrix)
    ...
    def line_to(x, y)
      point_2 = point(x, y)
      Line.new(@point, point_2).draw(@image, line_pixel)
      @point = point_2
      self
    end
    ...
    def fill_to(border)
      ...
      ::GD2::GD2FFI.send(:gdImageFillToBorder, @image.image_ptr, @point.x.to_i, @point.y.to_i, @image.color2pixel(border), pixel.to_i)
      self
    end
    ...
    def text_circle(top, bottom, radius, text_radius, fill_portion)
      TextCircle.new(get_font, @point, radius, text_radius, fill_portion, top, bottom).draw(@image, pixel)
    end
    ...
    private
      ...
      def line_pixel
        if @style && @brush
          ...
          if @dont_blend
            ::GD2::GD2FFI.send(:gdImageSetAntiAliasedDontBlend, @image.image_ptr, pixel.to_i, @dont_blend.to_i)
          else
